<?php 
  session_start(); 

  if (!isset($_SESSION['username'])) {
  	$_SESSION['msg'] = "You must log in first";
  	header('location: logup.php');
  }
  if (isset($_GET['logout'])) {
  	session_destroy();
  	unset($_SESSION['username']);
  	header("location: logup.php");
  }
?>
<!DOCTYPE html>
<html lang="en">
    <head>
        <meta charset="utf-8">
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.4.1/css/bootstrap.min.css">
        <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js"></script>
        <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.4.1/js/bootstrap.min.js"></script>
        <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.16.0/umd/popper.min.js"></script>
        <script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.5.2/js/bootstrap.min.js"></script>
        <style>
        </style>
    </head>
    <body>
        <div class="container-fluid">
            <div class="row">
                <div class="col-sm-4 text-left">
                    <button href="myCart.html" class="btn btn-primary" type="submit">My cart: 0 items</button>
                </div>
                <div class="col-sm-3 text-right " style="font: size 16px;background-color: rgb(245, 247, 248);">
                
                    <h2>SCHOOLIFY!</h2>
                </div>
                <div class="col-sm-5         text-right">
                    <button href="Sell.html" class="btn btn-primary" type="submit" href>Sell</button>
                    <button href="Accounts.html" class="btn btn-primary" type="submit" href>My Account</button>
                    <button href=" login.html"class="btn btn-primary" type="submit">Login</button>
                </div>
            </div>
        </div>
        <div class="container-fluid text-center">
            <div class="row">
                <div class="col-sm-2 text-"></div>
                <img src="data:image/jpeg;base64," alt="website logo img-circle " class="text-right img-circle col-sm-2">
                <h1 class="col-sm-6 text-left"style="font: size 16px;"></h1>
            </div>
            <div class="container-fluid text-right">
                <div class="col-sm-2 text-left"></div>

            <div class="col-sm-8 input-group text-center">
                <input type="search"  class="form-control rounded" placeholder="Search" aria-label="Search" size="10px"
                  aria-describedby="search-addon" />
                <button type="button" class="btn btn-outline-primary">search</button>
              </div>
            </div>
            
            
            <div class="container">

                <div class="row">
                            <div class="col-md-12 col-lg-4 col-sm-3">
                            <div class="card" style="background-color: rgb(220, 240, 247);">
                              <img class="card-img-top img-circle" src="oldbooks.jfif" alt="Card image cap">
                              <div class="card-block">
                                <h4 class="card-title">Old Books</h4>
                                <p class="card-text">browse old books here</p>
                                <a href="http://v4-alpha.getbootstrap.com/components/card/" class="btn btn-primary">View</a>
                            </div>
                            </div>
                        </div>
                    
                
                        <div class="col-md-12 col-lg-4 col-sm-3">
                            <div class="card card-inverse card-primary text-center" style="background-color: rgb(220, 240, 247);">
                              <img class="card-img-top img-circle" src="newbooks.jfif" alt="Card image cap" style="width: 284px;height: 184px;">
                              <div class="card-block">
                                <h4 class="card-title">New Books</h4>
                                <p class="card-text">Some quick example text to build on the card title </p>
                                <a href="http://v4-alpha.getbootstrap.com/components/card/" class="btn btn-primary">View</a>
                            </div>
                            </div>
                        </div>
                
                
                        <div class="col-md-12 col-lg-4 col-sm-3">
                            <div class="card card-inverse card-success text-center" style="background-color: rgb(220, 240, 247);;">
                              <img class="card-img-top img-circle" src="sc.jfif" alt="Card image cap" style="width: 274px;height:184px;">
                              <div class="card-block">
                                <h4 class="card-title">School Supplies</h4>
                                <p class="card-text">Some quick example text to build on the card title and</p>
                                <a href="http://v4-alpha.getbootstrap.com/components/card/" class="btn btn-primary">View</a>
                            </div>
                            </div>
                        </div>
                        
                        
                            
                </div>
                <div class="row">
                    
                    
                    <div class="col-md-12 col-lg-4 col-sm-3">
                        <div class="card" style="background-color: rgb(220, 240, 247);">
                          <img class="card-img-top img-circle" src="calc.gif" alt="Card image cap"style="width: 274px;height:184px;">
                          <div class="card-block">
                            <h4 class="card-title">Electronics</h4>
                            <p class="card-text">browse old books here</p>
                            <a href="http://v4-alpha.getbootstrap.com/components/card/" class="btn btn-primary">View</a>
                        </div>
                        </div>
                    </div>

                    <div class="col-md-12 col-lg-4 col-sm-3">
                        <div class="card" style="width: 360px;background-color: rgb(220, 240, 247);">
                          <img class="card-img-top img-circle" src="lab.jfif" alt="Card image cap"style="width: 274px;height:184px;">
                          <div class="card-block" >
                            <h4 class="card-title">Lab Supplies</h4>
                            <p class="card-text">browse old books here</p>
                            <a href="http://v4-alpha.getbootstrap.com/components/card/" class="btn btn-primary">View</a>
                        </div>
                        </div>
                    </div>

                    <div class="col-md-12 col-lg-4 col-sm-3">
                        <div class="card" style="background-color: rgb(220, 240, 247);">
                          <img class="card-img-top img-circle" src="gifts.jfif" alt="Card image cap"style="width: 274px;height:184px;">
                          <div class="card-block">
                            <h4 class="card-title">Gifts</h4>
                            <p class="card-text">browse old books here</p>
                            <a href="http://v4-alpha.getbootstrap.com/components/card/" class="btn btn-primary">View</a>
                        </div>
                        </div>
                    </div>
                </div>
          
            </div>


          
       
   

       
    </body>
</html>